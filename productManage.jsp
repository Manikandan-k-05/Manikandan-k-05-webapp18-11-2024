				...
				<div class="card">
					...
					<div class="card-body">
						<table id="productsTable" class="table table-bordered">
							<thead>
								<tr>
									<th>SL No</th> <!-- New column for SL No -->
									<th>Name</th>
									<th>Category</th>
									<th>Price</th>
									<th>Availability</th>
							...
							<tbody>
								<c:forEach var="product" items="${products}">
									<tr>
										<td></td> <!-- SL No will be inserted here -->
<%-- 										<td>${product.productId}</td>
 --%>										<td>${product.productName}</td>
										...
										<td><img src="uploads/${product.productImage}" class="product-image" alt="Product Image" /></td>
										<td>
											<form action="editProduct.jsp" method="GET" style="display: inline;">
												<input type="hidden" name="productId" value="${product.productId}" />
												<button class="btn btn-sm btn-danger btn-spacing"><i class="fas fa-edit"></i></button>
											</form>
											<form action="deleteProductAction.jsp" method="POST" style="display: inline;">
												<input type="hidden" name="productId" value="${product.productId}" />
												<button class="btn btn-sm btn-danger btn-spacing" type="submit"><i class="fas fa-trash-alt"></i></button>
											</form>
										</td>
									</tr>
								</c:forEach>
	...
	<script>
		...
		document.addEventListener("DOMContentLoaded", function () {
			...
			rows.forEach((row, index) => {
				const slNoCell = row.children[0]; // First cell in the row
				slNoCell.textContent = index + 1; // Set the serial number
			});
		});
	</script>
